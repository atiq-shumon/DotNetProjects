...
namespace Nop.Web.Controllers
{
    public partial class InstallController : BasePublicController
    {
        ...
        public ActionResult Index()
        {
            ...
            //var y = Request.Url;
            //string x = System.Web.HttpContext.Current.Session["EMAIL"].ToString();
        ...
        {
            string userEmail = getRequestUser();
            System.Web.HttpContext.Current.Session["useremail"] = userEmail;

            //Tofael Add code
            model.AdminEmail = userEmail;
            model.AdminPassword = model.AdminPassword;
            model.ConfirmPassword = model.ConfirmPassword;
            model.SqlAuthenticationType = "sqlauthentication";
            model.SqlConnectionInfo = "sqlconnectioninfo_values";
            string useremail = userEmail.Replace("@", "");
            useremail = useremail.Replace(".", "");
            model.SqlDatabaseName = "VialinkerCommerceApps_" + useremail;
            model.SqlServerCreateDatabase = true;
            model.SqlServerName = "173.248.150.248,1533";
            model.SqlServerPassword = "%Via#l!NkeR2o1S";
            model.SqlServerUsername = "sa";
            model.Collation = "SQL_Latin1_General_CP1_CI_AS";
            model.DataProvider = "sqlserver";
            //Tofael Added code end
        ...
        private string getRequestUser()
        {
            try
            {
                string currentIPAddress = System.Net.Dns.GetHostName();
                string text = string.Empty;
                //string filename = "useremail.txt";
                string filename = currentIPAddress + ".txt";
                string apppathuser = Path.Combine(MapPath("~/App_Data/"), filename);

                if (System.IO.File.Exists(apppathuser))
                {
                    text = System.IO.File.ReadAllText(apppathuser);
                    //HttpContext.Current.Session["emailuser"] = text;
                }
                return text;
            }
            catch { return null; }
