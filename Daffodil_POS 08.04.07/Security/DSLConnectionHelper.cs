...
namespace DSL.Common.Security
{
    ...
    internal static class DSLConnectionHelper
    {
        private const string s_connPrefix = "Data Source=";
        ...
        private static void BuildConnectionForFileName(string dbFileName)
        {
            ...
            if (isConnString)
            {
                _Connections.Add(dbFileName, new DSLConnectionHolder(conn));
                return;
            }
            ...
            if (Path.IsPathRooted(dbFileName))
            {
                ...
                _Connections.Add(dbFileName, new DSLConnectionHolder(new SqlConnection(s_connPrefix + dbFileName)));
                return;
            }
            ...
            _Connections.Add(dbFileName, new DSLConnectionHolder(new SqlConnection(s_connPrefix + filename)));
        }

        internal static int GetApplicationID(SqlConnection connection, string applicationName)
        {
            return GetApplicationID(connection, applicationName, false);
        }

        internal static int GetApplicationID(SqlConnection connection, string applicationName, bool createIfNeeded)
        {
            SqlCommand lookupCommand = new SqlCommand("SELECT ApplicationId FROM DSLApplications WHERE ApplicationName = @AppName", connection);
            lookupCommand.Parameters.Add(new SqlParameter("@AppName", applicationName));
            ...
            if (createIfNeeded)
            {
                SqlCommand createCommand = new SqlCommand("INSERT INTO DSLApplications (ApplicationName) VALUES (@AppName)",
                    connection);
                createCommand.Parameters.Add(new SqlParameter("@AppName", applicationName));
        ...
        internal static int GetPathID(SqlConnection connection, int applicationID, string path)
        {
            return GetPathID(connection, applicationID, path, false);
        }

        internal static int GetPathID(SqlConnection connection, int applicationID, string path, bool createIfNeeded)
        {
            SqlCommand lookupCommand = new SqlCommand("SELECT PathId FROM aspnet_Paths WHERE ApplicationId = @AppId AND Path = @Path",
                connection);
            lookupCommand.Parameters.Add(new SqlParameter("@AppId", applicationID));
            lookupCommand.Parameters.Add(new SqlParameter("@Path", path));
            ...
            if (createIfNeeded)
            {
                SqlCommand createCommand = new SqlCommand("INSERT INTO aspnet_Paths (ApplicationId, Path) VALUES (@AppID, @Path)",
                    connection);
                createCommand.Parameters.Add(new SqlParameter("@AppID", applicationID));
                createCommand.Parameters.Add(new SqlParameter("@Path", path));
        ...
        internal static Guid GetUserID(SqlConnection connection, int applicationID, string userName)
        {
            return GetUserID(connection, applicationID, userName, false, false, DateTime.Now);
        }

        internal static Guid  GetUserID(SqlConnection connection, int applicationID, string userName, bool createIfNeeded)
        {
            return GetUserID(connection, applicationID, userName, createIfNeeded, false, DateTime.Now);
        }

        internal static Guid  GetUserID(SqlConnection connection, int applicationID, string userName, bool createIfNeeded, bool newUserIsAnonymous)
        {
            return GetUserID(connection, applicationID, userName, createIfNeeded, newUserIsAnonymous, DateTime.Now);
        }

        internal static Guid  GetUserID(SqlConnection connection, int applicationID, string userName, bool createIfNeeded, bool newUserIsAnonymous, DateTime lastActivityDate)
        {
            Guid GID = Guid.NewGuid();

            //GID.ToString(). = "00000000-0000-0000-0000-000000000000";

            GID = (Guid)TypeDescriptor.GetConverter(GID).ConvertFromString("00000000-0000-0000-0000-000000000000");
            ...
            if (connection == null)
                return GID; // Wrong params!

            SqlCommand lookupCommand = new SqlCommand(@"SELECT UserId FROM aspnet_Users WHERE ApplicationId = @AppId AND UserName = @UserName",
                                                            connection);
            lookupCommand.Parameters.Add(new SqlParameter("@AppId", applicationID));
            ...
            {
                return (Guid )lookupResult;
            }

            if (createIfNeeded)
            {
                SqlCommand createCommand = new SqlCommand(@"INSERT INTO aspnet_Users " +
                ...
                createCommand.Parameters.Add(new SqlParameter("@LastActivityDate", new DateTime(lastActivityDate.Year, lastActivityDate.Month, lastActivityDate.Day, lastActivityDate.Hour, lastActivityDate.Minute, lastActivityDate.Second)));
        ...
        private static void EnsureValidMdbFile(string fileName)
        {
            ...
            try
            {
                conn = new SqlConnection(s_connPrefix + fileName);
                conn.Open();
            }
            catch
            {
    ...
    /// //////////////////////////////////////////////////////////////////////////////
    internal sealed class DSLConnectionHolder
    {
        ...
        //////////////////////////////////////////////////////////////////////////////
        //////////////////////////////////////////////////////////////////////////////
        internal DSLConnectionHolder(SqlConnection connection)
        {
