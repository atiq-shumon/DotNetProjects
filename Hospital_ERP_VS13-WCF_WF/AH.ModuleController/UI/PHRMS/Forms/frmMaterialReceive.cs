...
namespace AH.ModuleController.UI.PHRMS.Forms
{
    public partial class frmMaterialReceive : AH.Shared.UI.frmSmartFormStandard
    {
        ...
        private void LoadPODetailsByID()
        {
            ...
            txtPharmacyID.DataBindings.Add("Text", phrSr.GetPOList(txtPOID.Text).ToList(), "Warehouse.TypeID");
            txtPharmacyName.DataBindings.Add("Text", phrSr.GetPOList(txtPOID.Text).ToList(), "Warehouse.TypeName");
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dgvMrrDetails.Rows.Count; i++)
            {
                ...
                //if (Convert.ToInt32(dgvMrrDetails.Rows[i].Cells["GRNQty"].Value.ToString()) <= 0)                
                //{
                ...
                //    return;
                //}
                //if (phrSr.VarifyDuplicateBatch(txtPharmacyID.Text, (String)dgvMrrDetails.Rows[i].Cells["Drugid"].Value, (String)dgvMrrDetails.Rows[i].Cells["BatchNo"].Value) > 0)
                //{
                ...
                //    if (!r.IsMatch((String)dgvMrrDetails.Rows[i].Cells["ExpireDate"].Value))
                //    {
        ...
        private MaterialReceive populateMatReceive()
        {
            ...
            Warehouse phar = new Warehouse();
            phar.TypeID = txtPharmacyID.Text;
            mr.Warehouse = phar;
        ...
        private void dgvMrrDetails_SelectionChanged(object sender, EventArgs e)
        {
            if (dgvMrrDetails.Rows.Count > 0)
            {
                ...
                txtGenericName.Text = dgvMrrDetails.CurrentRow.Cells["Generic Name"].Value.ToString();
                txtGrnQty.Text = dgvMrrDetails.CurrentRow.Cells["GRNQty"].Value.ToString();
                txtRemQty.Text = dgvMrrDetails.CurrentRow.Cells["RemQty"].Value.ToString();
                txtpurRate.Text = dgvMrrDetails.CurrentRow.Cells["Rate"].Value.ToString();
                txtSlrate.Text = dgvMrrDetails.CurrentRow.Cells["SaleRate"].Value.ToString();
            }
