...
{
    public partial class frmMaterialReceive2 : AH.Shared.UI.frmSmartFormStandard
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dgvMrrDetails.Rows.Count; i++)
            {
                ...
                if ((String)dgvMrrDetails.Rows[i].Cells["GRNQty"].Value == "0")
                {
                    MessageBox.Show("Quantity cannot be zero");
                    return;
                }
                if (float.Parse(dgvMrrDetails.Rows[i].Cells["Rate"].Value.ToString()) > float.Parse(dgvMrrDetails.Rows[i].Cells["SaleRate"].Value.ToString()))
                {

                    MessageBox.Show("Sales Rate can not be less than Purchase Rate");
                    return;
                }
                if (Convert.ToInt32(dgvMrrDetails.Rows[i].Cells["GRNQty"].Value) > Convert.ToInt32(dgvMrrDetails.Rows[i].Cells["RemQty"].Value))
                {
                    MessageBox.Show("Mrr Qty can not be more than  RemQty");
                    return;
                }
               
                if ((String)dgvMrrDetails.Rows[i].Cells["BatchNo"].Value == "")
                {
                    MessageBox.Show("Please fill Batch No");
                    return;
                }
                if(Convert.ToString(dgvMrrDetails.Rows[i].Cells["BatchNo"].Value).Length > 12)
                {
                    MessageBox.Show("Batch no must be 12 Digits Long!");
                    return;
                }
                if ((String)dgvMrrDetails.Rows[i].Cells["ExpireDate"].Value != "  /  /    ")
                {
                    if (Convert.ToDateTime(dgvMrrDetails.Rows[i].Cells["ExpireDate"].Value) < Convert.ToDateTime(DateTime.Now.ToString("dd/MM/yyyy")))
                    {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in dgvPODetails.Rows)
            {
                ...
                if (isSelected)
                {
                    ...
                    {
                        if (Convert.ToBoolean(dgvPODetails.Rows[k].Cells["checkBoxColumn"].Value) == true)
                        {
                            if (Convert.ToInt32(dgvPODetails.Rows[k].Cells["RemQty"].Value.ToString()) == 0)
                            {
                    ...
                    dgvMrrDetails.Rows.Add(row.Cells[0].Value.ToString(),
                    ...
                    //dgvMrrDetails.Columns[10].Visible = false;
                    //dgvMrrDetails.Columns[15].Visible = false;
                   ...
                   // dgvMrrDetails.Rows[1].Cells["ExpireDate"].Value = oDTime;
                   // dgvMrrDetails.Columns["CompanyName"].ValueType=oDTime
                    ...
                    //this.dgvMrrDetails.Controls.Add(dtp);
                    ////set its location and size to fit the cell
                    //dtp.Location = this.dgvMrrDetails.GetCellDisplayRectangle(0, 1, true).Location;
                    //dtp.Size = this.dgvMrrDetails.GetCellDisplayRectangle(0, 3, true).Size;



                   // DataGridViewTextBoxCell TextBoxCell = new DataGridViewTextBoxCell();
                   // dgvMrrDetails.Columns["ExpireDate"].ValueType = TextBoxCell;
                   // this.dataGridView1[0, 1].Value = "some text";
                }


            }
           // SmartDateTextBox[] temp = new SmartDateTextBox[2];

          
           // for (counter = 0; counter < dgvMrrDetails.Rows.Count; counter++)
            //{
            //  // dgvMrrDetails.Rows[0].sele
            //   // string counter_temp = ;
            //   // var temp=counter.ToString();
               
                
            //    //oDTime.Format = DateTimePickerFormat.Custom;
            //    //oDTime.CustomFormat = "hh:mm tt";
            //    // oDTime.Visible = false;
            //    //temp[counter].Width = 100;
            //    //temp[counter].Height = 80;
            //    ////oDTime.ShowUpDown = true;

            //    dgvMrrDetails.Controls.Add(temp[counter]);
            //    //temp[counter].Visible = true;
            //    temp[counter].Location = dgvMrrDetails.GetCellDisplayRectangle(18, counter, false).Location;
                
            //}
            //dgvMrrDetails.Columns["POQty"].HeaderText = "Mrr Qty";
        ...
        private void dgvMrrDetails_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //    dgvMrrDetails.Controls.Add(oDateTimePicker);
            //    oDateTimePicker.Visible = false;
            //    oDateTimePicker.Format = DateTimePickerFormat.Short;  //
            //    oDateTimePicker.TextChanged += new EventHandler(dgvMrrDetails_OnTextChange);
            //    oDateTimePicker.Visible = true;
            //    Rectangle oRectangle = dgvMrrDetails.GetCellDisplayRectangle(e.ColumnIndex, e.RowIndex, true);
            //    oDateTimePicker.Size = new Size(oRectangle.Width, oRectangle.Height);
            //    oDateTimePicker.Location = new Point(oRectangle.X, oRectangle.Y);
            //    oDateTimePicker.CloseUp += new EventHandler(dgvMrrDetails_CloseUp);

            //}
        }
        private void dgvMrrDetails_OnTextChange(object sender, EventArgs e)
        {
            dgvMrrDetails.CurrentCell.Value = oDateTimePicker.Text.ToString();
        }

        void dgvMrrDetails_CloseUp(object sender, EventArgs e)
        {
            oDateTimePicker.Visible = false;
        }
        ...
        private void dgvMrrDetails_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            //if (e.ColumnIndex == 18 && e.RowIndex >= 0)
            //{
            //    if (dgvMrrDetails.CurrentCell.Value == null || dgvMrrDetails.CurrentCell.Value.ToString().Trim() == "" || dgvMrrDetails.CurrentCell.Value.ToString() == "  /  /    ")
            ...
            //    else
            //    {
            //        temp.Text = dgvMrrDetails.CurrentCell.Value.ToString();
            //    }
            //}
        }
