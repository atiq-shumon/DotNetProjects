        ...
        private void frmReportViewer_Load(object sender, EventArgs e)
        {
            ...
            switch (selector)
            {
                ...
                case ViewerSelector.BarcodeLevel:
                    List<VisitorsSlipRO> objS = fnbs.GetEmpStaffItem(Where,"","").ToList();
                    if (objS.Count > 0)
                    {
                        ...
                        this.secondParameter = "";
                        InitialiseLabels(rpt1);
                        rpt1.SetDataSource(fnbs.GetEmpStaffItem(Where,"","").ToList());
                ...
                case ViewerSelector.ColelctionFromVisitors:
                    if (intSelection == 2)
                    {
                        List<VisitorsSlipRO> ds1 = fnbs.GetVisitorsItemDetails("0", "VisitorsNew", strFromDate, strToDate).ToList();
                        {
                            if (ds1.Count > 0)
                            {
                                ...
                                this.ReportSecondParameter = strFromDate + " to " + strToDate;
                                InitialiseLabels(rpt1);
                                rpt1.SetDataSource(fnbs.GetVisitorsItemDetails("", "VisitorsNew", strFromDate, strToDate).ToList());
                    ...
                    List<DailyCollectionRO> ds2 = fnbs.LoadStaffCollection("", "Staff Collection", strFromDate, strToDate).ToList();
                    {
                        if (ds2.Count > 0)
                        {
                            if (intSelection == 1)
                            {
                                rptDailyCollectionSumm rptDailyColl = new rptDailyCollectionSumm();
                                rpt1 = (ReportDocument)rptDailyColl;
                                this.reportTitle = "Collection From Staff(Summary)";
                                this.ReportSecondParameter = strFromDate + " to " + strToDate;
                                InitialiseLabels(rpt1);
                            ...
                            else
                            {
                                ...
                                rpt1 = (ReportDocument)rptDailyColl;
                                this.reportTitle = "Collection From Staff(Details)";
                                this.ReportSecondParameter = strFromDate + " to " + strToDate;
                                InitialiseLabels(rpt1);
                                rpt1.SetDataSource(fnbs.LoadStaffCollection("", "Details", strFromDate, strToDate).ToList());
                            }
                ...
                case ViewerSelector.Inventory:
                    List<InventoryRO> ds5 = fnbs.GetfnbInventoryDetails(strFromDate, strToDate).ToList();
                    {
                        if (ds5.Count > 0)
                        {
                            rptInventory rptInv = new rptInventory();
                            rpt1 = (ReportDocument)rptInv;
                            this.reportTitle = "Stock Summary ";
                            this.ReportSecondParameter = strFromDate + " to " + strToDate;
                            InitialiseLabels(rpt1);
                            rpt1.SetDataSource(fnbs.GetfnbInventoryDetails(strFromDate, strToDate).ToList());
                            crystalReportViewer1.ReportSource = rpt1;
                            ShowReport(rpt1, this.isPrintDirect, " " + "");
                        }
                    }
                ...
                case ViewerSelector.MonthWiseMEalReg:
                    if (intAll==0)
                    {
                        List<MonthwiseMealRegisterRO> Mr = fnbs.GetMonthWiseMealRegister(Where,where1 , strFromDate, strToDate).ToList();
                        {
                            if (Mr.Count > 0)
                            {
                                rptMonthWiseMealRegister rptInv = new rptMonthWiseMealRegister();
                                rpt1 = (ReportDocument)rptInv;
                                this.reportTitle = "Monthwise Meal Register ";
                                this.ReportSecondParameter = strFromDate + " to " + strToDate;
                                InitialiseLabels(rpt1);
                                rpt1.SetDataSource(fnbs.GetMonthWiseMealRegister(Where, where1, strFromDate, strToDate).ToList());
                                rpt1.SetParameterValue("title", strParam);
                                crystalReportViewer1.ReportSource = rpt1;
                    ...
                    {
                        List<MonthwiseMealRegisterRO> Mr = fnbs.GetMonthWiseMealRegisterAll(Where, where1, strFromDate, strToDate).ToList();
                        {
                            if (Mr.Count > 0)
                            {
                                rptMonthWiseMealRegister rptInv = new rptMonthWiseMealRegister();
                                rpt1 = (ReportDocument)rptInv;
                                this.reportTitle = "Monthwise Meal Register All ";
                                this.ReportSecondParameter = strFromDate + " to " + strToDate;
                                InitialiseLabels(rpt1);
                                rpt1.SetDataSource(fnbs.GetMonthWiseMealRegisterAll("", "", strFromDate, strToDate).ToList());
                                rpt1.SetParameterValue("title", strParam);
                                crystalReportViewer1.ReportSource = rpt1;
